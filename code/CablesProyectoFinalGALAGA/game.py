import time
from Code.ProyectoFinalGALAGA.colours import (BLACK, WHITE, CYAN, ORANGE, BLUE, NAVY, YELLOW,
                     PURPLE, LEVEL_COLOURS)
from Code.ProyectoFinalGALAGA.player import Player
from Code.ProyectoFinalGALAGA.enemy import Enemy
from Code.ProyectoFinalGALAGA.bullet import Bullet
from Code.ProyectoFinalGALAGA.asteroid import Asteroid, Guardian, DiveBomber
from Code.ProyectoFinalGALAGA.sprites.sprites import (PLAYER_W, PLAYER_H, ALIEN_W, ALIEN_H,
...
class Game:
    def __init__(self, display, buzzer, btn_fire, btn_pause, imu):
    ...
    def _save_positions(self):
        """Save current positions so we can erase them next frame."""
    ...
    def _next_level(self):
        # Incrementamos el nivel directamente
        self.level += 1
        self._set_difficulty()

        # ANUNCIO VISUAL: Limpiamos la pantalla para mostrar la transición de nivel
    ...
    def _game_over_screen(self):
        ...
        while not self.btn_fire.is_pressed():
            self.buzzer.update()
            time.sleep_ms(50)
    ...
    def run(self):
        while True:
            ...
            while True:
                ...
                self.asteroid.update(self.frame_count,
                                     self.asteroid_interval, self.asteroid_speed)
                self.guardian.update(self.frame_count)
                self.dive_bomber.update(self.frame_count,
                                        self.dive_bomber_interval,
                                        self.dive_bomber_speed,
                                        self.enemies)
                self._move_enemies()
