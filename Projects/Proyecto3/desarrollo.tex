...
activo con temporización precisa según el periodo configurado, mientras que el \textbf{Núcleo 0} 
...
Se selecciona una frecuencia de corte $f_c \approx 39.6$ Hz para el filtro pasa altas. 
Para esto, se tomo en cuenta lo siguiente:

\begin{enumerate}
    \item \textbf{Criterio de Nyquist:} Se cumple $f_c \ll f_N$ para el muestreo definido, garantizando estabilidad y correcta representación digital.

    \item \textbf{Relación:} La relación $f_c T_{HP} = (39.6)(1\times 10^{-3}) = 0.0396$ se ubica en el rango donde la distorsión por la transformada bilineal es baja y controlable.
...
Seleccionando $C = 100$ nF (serie E24), se calcula la resistencia necesaria para obtener $f_c \approx 39.6$ Hz:
\begin{equation}
    R = \frac{1}{2\pi f_c C} = \frac{1}{2\pi (39.6)(100 \times 10^{-9})} = \frac{1}{2.487 \times 10^{-5}} = 40210\;\Omega \approx 40.2\;\text{k}\Omega
\end{equation}

Se utiliza una resistencia de $R = 40.2\;\text{k}\Omega$ (valor teórico), lo cual resulta en una frecuencia de corte real de:
\begin{equation}
    f_{c,\text{real}} = \frac{1}{2\pi (40200)(100 \times 10^{-9})} \approx 39.6\;\text{Hz}
\end{equation}
...
Dividiendo entre $RC$:
\begin{equation}
    \boxed{H(s) = \frac{s}{s + \omega_c}, \qquad \omega_c = 2\pi(39.6) = 248.7\;\text{rad/s}}
    \label{eq:hp_hs}
\end{equation}
...
Para la discretización se usa un periodo de muestreo $T_{HP} = 1\;\text{ms}$.

\begin{equation}
    \Omega_a = \tan\left(\pi f_c T_{HP}\right) = \tan\left(\pi (39.6)(1\times 10^{-3})\right) = \tan(0.12436) = 0.125
    \label{eq:omega_a_hp}
\end{equation}
...
\begin{align}
    A_0 &= \frac{1}{1 + \Omega_a} = \frac{1}{1 + 0.125} = \frac{1}{1.125} = 0.8889 \\[6pt]
    A_1 &= -\frac{1}{1 + \Omega_a} = -0.8889 \\[6pt]
    B_1 &= \frac{1 - \Omega_a}{1 + \Omega_a} = \frac{1 - 0.125}{1 + 0.125} = \frac{0.875}{1.125} = 0.7778
\end{align}

La función de transferencia discreta resulta:
\begin{equation}
    \boxed{H(z) = \frac{0.8889\,z - 0.8889}{z - 0.7778}}
    \label{eq:hp_hz}
\end{equation}
...
Sustituyendo los valores numéricos:
\begin{equation}
    y(k) = 0.8889\,u(k) - 0.8889\,u(k-1) + 0.7778\,y(k-1)
\end{equation}
...
\begin{table}[H]
    \centering
    \caption{Coeficientes del filtro pasa altas ($f_c \approx 39.6$ Hz).}
    \label{tab:hp_coeff}
    \begin{tabular}{|c|c|c|}
        \hline
        \rowcolor{headblue}\textcolor{white}{\textbf{Coeficiente}} & \textcolor{white}{\textbf{Expresión}} & \textcolor{white}{\textbf{Valor numérico}} \\
        \hline
        $A_0$ & $\dfrac{1}{1 + \Omega_a}$ & $0.8889$ \\[10pt]
        \hline
        $A_1$ & $-\dfrac{1}{1 + \Omega_a}$ & $-0.8889$ \\[10pt]
        \hline
        $A_2$ & --- & $0.0$ \\[4pt]
        \hline
        $B_1$ & $\dfrac{1 - \Omega_a}{1 + \Omega_a}$ & $0.7778$ \\[10pt]
        \hline
        $B_2$ & --- & $0.0$ \\[4pt]
        \hline
    \end{tabular}
...
\begin{itemize}
    \item Frecuencia de corte: $f_c \approx 39.6$ Hz, con $T_{HP} = 1\;\text{ms}$.
    \item Pre-distorsión: $\Omega_a = 0.125$.
    \item Coeficientes: $A_0 = 0.8889$, $A_1 = -0.8889$, $B_1 = 0.7778$.
    \item Ecuación en diferencias: $y(k) = 0.8889\,u(k) - 0.8889\,u(k-1) + 0.7778\,y(k-1)$.
...
A continuación se definen los parámetros fundamentales del sistema. Para el pasa bajas se usa \texttt{FS\_LP = 8000}~Hz con \texttt{PERIOD\_US\_LP = 125}~$\mu$s. Para el pasa altas se definen sus propias constantes \texttt{FS\_HP} y \texttt{PERIOD\_US\_HP} con su temporización correspondiente. Estos valores se guardan en constantes para que sean fáciles de modificar si en algún momento se quisiera cambiar la tasa de muestreo de cada filtro. También se definen \texttt{PRINT\_SKIP\_LP = 2} y \texttt{PRINT\_SKIP\_HP = 1}, que indican cada cuántas muestras se envían datos al puerto serial para su visualización; esto es necesario porque imprimir todas las muestras desbordaría el buffer USB y provocaría que los comandos dejaran de responder.
...
La parte central del programa es la función \texttt{filter\_core}, que se ejecuta en el Núcleo~1 del procesador. Esta función recibe como argumentos todo lo que necesita para operar: la lista de estado compartido, el objeto del ADC, el módulo de tiempo, todos los coeficientes de ambos filtros, los periodos de muestreo y los saltos de impresión. De esta forma, la función no hace ninguna referencia a nombres globales del módulo, lo que evita el error de visibilidad que tiene el RP2350 con los hilos. Dentro de un ciclo infinito, mientras la bandera de ejecución esté activa, la función registra el tiempo de inicio, lee el valor del ADC y lo normaliza a $[0,1]$, y evalúa la ecuación en diferencias del filtro seleccionado. Cada cierto número de muestras (determinado por \texttt{PRINT\_SKIP\_LP} o \texttt{PRINT\_SKIP\_HP}, según el filtro), guarda los valores de entrada y salida en la lista compartida y activa la bandera de datos nuevos para que el Núcleo~0 los imprima. Al final de cada iteración, calcula cuánto tiempo tardó el procesamiento y espera el tiempo restante para completar exactamente el periodo configurado.
