...
\begin{itemize}
    \item \textbf{Núcleo~1:} Ejecuta exclusivamente la ecuación en diferencias a $f_s = 8000$ Hz. Este núcleo \textbf{no realiza impresión por serial} para evitar saturar el buffer USB, lo cual garantiza que el Núcleo~0 siempre pueda recibir comandos (como \texttt{STOP}).
    \item \textbf{Núcleo~0:} Gestiona la comunicación serial USB mediante lectura \textbf{no bloqueante} (\texttt{select.poll()}) y se encarga de imprimir los datos del filtro a una tasa reducida de 200 muestras/segundo, compatible con la velocidad del puerto serial.
\end{itemize}

Los comandos disponibles son: \texttt{START} (iniciar filtrado), \texttt{STOP} (detener), \texttt{LP} (seleccionar pasa bajas), \texttt{HP} (seleccionar pasa altas), \texttt{FREQ~<hz>} (cambiar frecuencia de la onda cuadrada de prueba) y \texttt{STATUS} (consultar estado).
